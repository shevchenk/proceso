    ...
    GuardarPreTramite:function(data){
        $.ajax({
            ...
            success : function(obj) {
                $(".overlay,.loading-img").remove();
                if(obj.rst==1){
                    $('#FormCrearPreTramite').find('input[type="text"], input[type="hidden"], input[type="email"],textarea,.select').val('');
                    msjG.mensaje("success","Registrado",3000); 
                    $("#t_referidos").dataTable().fnDestroy();
                    $("#tb_referidos, #tb_numareas, #tb_archivos").html('');
                    $('.select').multiselect('refresh');
                    $("#pdf_img").attr('src', 'Config/default.png');
