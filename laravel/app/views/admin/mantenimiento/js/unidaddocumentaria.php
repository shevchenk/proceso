...
BtnEditar=function(btn,id){
    ...
    DocumentosG.estado=$(tr).find("td:eq(2)>span").attr("data-estado");
    $("#BtnEditar").click();
};
...
GeneraFn=function(row,fn){ // No olvidar q es obligatorio cuando queire funcion fn
    ...
    if(typeof(fn)!='undefined' && fn.col==2){
        var estadohtml='';
        let complementarios = "<input type='hidden' name='slct_tipo' value='"+row.tipo+"'>"+
        ...
        estadohtml='<span id="'+row.id+'" onClick="activar('+row.id+')" data-estado="'+row.estado+'" class="btn btn-danger">Inactivo</span>'+complementarios;
        if(row.estado==1){
            estadohtml='<span id="'+row.id+'" onClick="desactivar('+row.id+')" data-estado="'+row.estado+'" class="btn btn-success">Activo</span>'+complementarios;
        }
        return estadohtml;
    }
}
