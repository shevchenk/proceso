...
validaPersonas=function(){
 /*   $('#form_personas_modal [data-toggle="tooltip"]').css("display","none");
    ...
    $('#t_cargoPersona li input[type="hidden"]').each(function(index, data){
        console.log($.trim( $("#slct_areas_"+data.value).val() ), r);
        if( r == true && $.trim( $("#slct_areas_"+data.value).val() ) == "" ){
            alert("Seleccione Áreas de la fila "+(index*1+1));
            r=false;
        }
    });
    return r;
