    ...
    if(data.length > 0){
        ...
        $.each(data,function(index, el) {
            ...
            if(el.doc_digital_id!=null){
                link='<a class="btn btn-default btn-sm" href="doc_digital/'+el.doc_digital_id+'" target="_blank" data-titulo="Previsualizar"><i class="fa fa-eye fa-lg"></i> </a>';
            }else{
                link='';
            }

            var dd = (parent>0?'style="background-color: rgba(255, 244, 162, 0.42);"':'style=""');
            var dd2 =  (parent>0?'&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa fa-angle-double-right"></i>':'');

            html+="<tr data-id="+cont+" "+dd+" data-parent="+parent+" data-level="+child+">";
            html+=    "<td data-column=name>"+dd2+referido+link+"</td>";
            html+=    "<td>"+fhora+"</td>";
            html+=    "<td>"+proc+"</td>";
            html+=    "<td>"+area+"</td>";
            html+=    "<td>"+nord+"</td>";
