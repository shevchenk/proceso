   ...
   public function getExporttramiteconclu()
    {
        ...
        if ( Input::has('proceso') ) {
            ...
            $sql = "SELECT GROUP_CONCAT(id) ids
                    FROM rutas_flujo 
                    WHERE flujo_id = ".Input::get('proceso');
            $flujo= DB::select($sql);
            $wher_3.= " AND (r.flujo_id ='".Input::get('proceso')."' OR FIND_IN_SET(rd.ruta_flujo_id_dep,'".$flujo[0]->ids."')>0 ) ";
        }
...
/*          if($result){
   ...
   /*   if( Request::ajax() ){*/
    ...
    public function postObtenerqr() {
      ...
      $png = QrCode::format('png')->margin(0)->size($size)->generate("http://proceso.jssoluciones.pe/documentodig/vistauserqr/".Input::get('rol_id')."/".Input::get('area_id')."/".Input::get('dni')."/".Input::get('tamano')."/".Input::get('tipo'));
